...
public class Prospector : MonoBehaviour
{
    ...
    public float reloadDelay = 2f;// Задержка между раундами 2 секунд
    ...
    List<CardProspector> ConvertListCardsToListCardProspectors(List<Card> lCD)
    {
        ...
        {
            tCP = tCD as CardProspector;//Ключевое слово as пытается преобразовать тип с Card на CardProspector
            lCP.Add(tCP);
        }
        return (lCP);
    }
    ...
    CardProspector Draw()
    {
        ...
        drawPile.RemoveAt(0);// Удалить из Listo drawPile
    ...
    public void CardClicked(CardProspector cd)
    {
        ...
        switch (cd.state)
        {
            ...
            case eCardState.tableau:
                ...
                if (!validMath) return; // Выйти, если карта не может перемещаться
    ...
    public bool AdjacentRank(CardProspector c0, CardProspector c1)
    {
        ...
        //если достоинство карт отличается на 1, правило старшинства соблюдено
    ...
    //преобразует номер слота layoutID в эеземпляр CardProspector с этим нормером
